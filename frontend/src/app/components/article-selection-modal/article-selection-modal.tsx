  ...
  return (
    <>
      <Modal show={true} className="full-screen">
        ...
        <Modal.Body>
          {xmlArticles &&
            xmlArticles.map((value, index) => (
              <div key={index} className={'m-1 articles'}>
                <label className={'d-flex align-content-center'}>
                  <input type={'checkbox'} checked={articleChecked[index]} onChange={() => checkHandler(index)} />
        ...
        <Modal.Footer>
          ...
          <Button
            className="save float-end m-2 mt-3"
            onClick={() => {
          ...
          <Button
            className="cancel float-end m-2 mt-3"
            onClick={() => {
