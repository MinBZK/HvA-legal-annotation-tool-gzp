      ...
      if (selectedLawClass) {
        ...
        if (subAnnotationsResponse.ok) {
          ...
          if (childAnnotations.length > 0) {
            ...
            const annotationsToDelete = childAnnotations.filter((childAnnotation: any) => childAnnotation.lawClassId !== lawClassId).map((childAnnotation: any) => childAnnotation.id);
              ...
              annotationsToDelete.map(async (annotationId: any) => {
                try {
                  ...
                  if (!deleteAnnotationResponse.ok) {
                    console.error('Failed to delete child annotation:', deleteAnnotationResponse.status, deleteAnnotationResponse.statusText);
                  }
                } catch (error) {
          ...
          console.error('Failed to fetch subannotations:', subAnnotationsResponse.status, subAnnotationsResponse.statusText);
        }
      }
    } catch (error) {
    ...
    <div>
      {open && (
        <div className={`${css.annotationInfo} ${isEditing ? css.editing : ''}`}>
          <div className={css.iconRow}>
            <FaEdit className={css.iconCol} id={'iconEdit'} style={isEditing ? { display: 'none' } : { display: 'block' }} onClick={() => setIsEditing(true)} />
          </div>
            ...
            {isEditing ? (
              <Dropdown>
                <Dropdown.Toggle
                  ...
                  style={{
                    color: 'black',
                    backgroundColor: editLawClass ? classes.find((law) => law.name === editLawClass)?.color || '' : '',
                  }}
                >
                ...
                <Dropdown.Menu className="dropdown">
                  {classes.map((law, index) => (
                    <Dropdown.Item key={index} onClick={() => handleSelectLaw(law.name)} active={editLawClass === law.name} style={{ backgroundColor: law.color, color: 'black' }}>
                      {law.name}
                    </Dropdown.Item>
                  ))}
                </Dropdown.Menu>
              ...
              <Form.Group controlId="exampleForm.ControlInput2">
                <Dropdown>
                  <Dropdown.Toggle className="dropdown" variant="secondary" id="dropdown-basic">
                    {editTerm?.definition ? editTerm.definition : <>Selecteer</>}
                  </Dropdown.Toggle>
                  ...
                  <Dropdown.Menu className="dropdown">
                    ...
                    {terms.map((term, index) => (
                      <Dropdown.Item key={index} onClick={() => setEditTerm(term)} active={editTerm?.definition === term.definition} style={{ color: 'black' }}>
                        {term.definition}
                      </Dropdown.Item>
                    ))}
          ...
          {annotation.parentAnnotation != null ? (
            <div className={css.row}>
              ...
              <h4 className={`${css.rightCol} ${css.annotationName} ${css.childAnnotation}`} style={{ background: annotation.parentAnnotation.lawClass?.color }}>
                {annotation.parentAnnotation.selectedWord}
              </h4>
            </div>
          ) : (
            ''
          ...
          {isEditing && (
            <div className={`${css.buttonsRight}`}>
              <button className={`${css.save}`} onClick={() => setIsConfirmModalOpen(true)}>
                Opslaan
              </button>
              <button className={`${css.cancel}`} onClick={() => setIsEditing(false)}>
                Annuleer
              </button>
              <button className={`${css.delete}`} onClick={() => setIsDeleteModalOpen(true)}>
                Verwijderen
              </button>
            </div>
          )}
      ...
      >
        ...
        <Modal.Body>
          ...
          <Button variant="success" onClick={checkValues}>
            Ja
          </Button>
        </Modal.Body>
      </Modal>
      ...
      >
        ...
        <Modal.Body>
          ...
          <Button variant="success" onClick={checkDelete}>
            Ja
          </Button>
        </Modal.Body>
      </Modal>
