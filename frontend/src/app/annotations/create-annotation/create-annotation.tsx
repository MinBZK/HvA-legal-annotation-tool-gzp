...
const CreateAnnotation: FC<PopupProps> = ({ selectedText1, selectedText2, tempId1, tempId2, onSetActiveSelection, onClose, onAnnotationSaved, currentUser, addRelation, cancel }) => {
    ...
    <div>
      ...
      <Form className="annotationInfo">
          ...
          <Dropdown>
            ...
            <Dropdown.Menu className="dropdown">
              {classes.map((law, index) => (
                <Dropdown.Item key={index} onClick={() => handleSelectLaw(law.name)} active={annotation?.lawClass && law.name === annotation.lawClass.toString()} style={{ backgroundColor: law.color, color: 'black' }}>
                  {law.name}
                </Dropdown.Item>
              ))}
            </Dropdown.Menu>
        ...
        <Form.Group controlId="exampleForm.ControlInput1">
          <Form.Label className="padding">
            <b>Notitie</b>
          </Form.Label>
          <Form.Control className={'text-input'} as="textarea" type="text" placeholder="Type hier uw notitie..." value={annotation?.text} onChange={(e) => handleNote(e.target.value)} />
        </Form.Group>
        ...
        <Form.Group controlId="exampleForm.ControlInput2">
          ...
          <Dropdown>
            ...
            <Dropdown.Menu className="dropdown">
              {terms.map((term, index) => (
                <Dropdown.Item key={index} onClick={() => handleTerm(term)} active={annotation?.term?.definition === term.definition} style={{ color: 'black' }}>
                  {term.definition}
                </Dropdown.Item>
              ))}
          ...
          <Modal show={showModal} onHide={() => setShowModal(false)}>
            ...
            <Modal.Body>
              <Form.Control as="textarea" placeholder="Vul begrip in" value={newTerm.definition} onChange={(e) => setNewTerm({ ...newTerm, definition: e.target.value, reference: annotation?.selectedWord })} />
            </Modal.Body>
            <Modal.Footer>
              <Button
                variant="secondary"
              ...
              {showSubAnnotationForm && (
                <>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    >
                      Juridische subklasse: {subAnnotationDetails.lawClass?.name}
                    </Form.Label>
                    <Button
                  ...
                  <Form.Group>
                    ...
                    <Form.Control type="text" readOnly value={subAnnotationDetails.selectedWord} onChange={(e) => handleSubAnnotationDetailChange('selectedWord', e.target.value)} />
                  </Form.Group>
                  <Form.Group>
                    <Form.Label>Notitie</Form.Label>
                    <Form.Control type="text" value={subAnnotationDetails.text} onChange={(e) => handleSubAnnotationDetailChange('text', e.target.value)} />
                  </Form.Group>
                  <Form.Group>
                    <Form.Label>
                      <b>Begrip</b>
                    ...
                    <Dropdown>
                      ...
                      <Dropdown.Menu className="dropdown">
                        {terms.map((term, index) => (
                          <Dropdown.Item key={index} onClick={() => handleTerm(term)} active={subAnnotationDetails?.term?.definition === term.definition} style={{ color: 'black' }}>
                            {term.definition}
                          </Dropdown.Item>
                        ))}
          ...
          <Modal show={showSubModal} onHide={() => setSubShowModal(false)}>
            ...
            <Modal.Body>
              <Form.Control as="textarea" placeholder="Vul begrip in" value={subNewTerm.definition} onChange={(e) => setSubNewTerm({ ...subNewTerm, definition: e.target.value, reference: subAnnotationDetails?.selectedWord })} />
            </Modal.Body>
            <Modal.Footer>
      ...
      <div className={`${css.buttonsRight}`}>
        ...
        {parentAnnotationTagsExists && (
          <button className={`${css.save}`} onClick={() => handleFinish(false)}>
      ...
      <Modal show={showWarningModal} onHide={() => setShowWarningModal(false)}>
        ...
        <Modal.Body>Weet u zeker dat u de annotaties wil opslaan zonder alle verplichte relaties te leggen?</Modal.Body>
        <Modal.Footer>
          <Button
            variant="secondary"
            onClick={() => {
      ...
      <Modal show={showInputWarningModal} onHide={() => setShowInputWarningModal(false)}>
        ...
        <Modal.Body>Weet u zeker dat u wil afronden zonder de ingevulde velden op te slaan?</Modal.Body>
        <Modal.Footer>
          ...
          <Button
            variant="primary"
            onClick={() => {
              setShowInputWarningModal(false);
              handleClose();
              onClose();
            }}
          >
